...
int main(const int argumentCount, char** arguments)
{
    ...
    {
        const vpost::FullSceneDepthQuad leftEye =
            vpost::ResolveFullSceneDepthQuad(
                true,
                0.0f,
                0.0f,
                1872.0f,
                2016.0f);
        const vpost::FullSceneDepthQuad rightEye =
            vpost::ResolveFullSceneDepthQuad(
                true,
                1872.0f,
                0.0f,
                1872.0f,
                2016.0f);
        const vpost::FullSceneDepthQuad retailViewport =
            vpost::ResolveFullSceneDepthQuad(
                false,
                320.0f,
                180.0f,
        ...
        Check(
            NearlyEqual(leftEye.x, 0.0f) &&
                NearlyEqual(leftEye.y, 0.0f) &&
                NearlyEqual(leftEye.width, 1872.0f) &&
                NearlyEqual(leftEye.height, 2016.0f) &&
                NearlyEqual(rightEye.x, 0.0f) &&
                NearlyEqual(rightEye.y, 0.0f) &&
                NearlyEqual(rightEye.width, 1872.0f) &&
                NearlyEqual(rightEye.height, 2016.0f),
            "issue #78 V117 should clear float-Z in viewport-local coordinates for every packed stereo eye");
        Check(
            NearlyEqual(retailViewport.x, 320.0f) &&
                NearlyEqual(retailViewport.y, 180.0f) &&
                NearlyEqual(retailViewport.width, 1280.0f) &&
                NearlyEqual(retailViewport.height, 720.0f),
            "issue #78 V117 should preserve original full-scene mesh coordinates outside same-frame stereo");
    }
        ...
        Check(
            rendererScene.find(
                "KISAK_SP_VR_ISSUE78_FLOAT_Z_VIEWPORT_V117") !=
                    std::string::npos &&
                rendererScene.find(
                    "ResolveFullSceneDepthQuad(") !=
                    std::string::npos &&
                rendererScene.find(
                    "VR_D3D9IsSameFrameStereoEnabled(),") !=
                    std::string::npos,
            "issue #78 V117 must use viewport-local float-Z clear meshes only during same-frame stereo");
        Check(
            clientInput.find(
