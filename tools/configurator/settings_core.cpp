...
const std::vector<SettingDefinition> kCatalog = {
    ...
    Choice(
        ...
        "Native preserves the Quest 3 eye and scope layout. Performance uses the verified lower mode. Pimax Crystal Light reserves both reported 3330 px eyes plus the 1024 px scope panel.",
        SettingPage::Graphics,
        "6016x2688",
        {{"6016x2688", "Native 6016 x 2688"}, {"4768x2016", "Performance 4768 x 2016"}, {"7684x3128", "Pimax Crystal Light 7684 x 3128"}}),
    Choice(
    ...
    const SettingsMap& values)
{
        ...
        valueOf("KISAK_VR_OUTPUT_SCALE") != "1.00")
    {
        messages.push_back({
            ValidationMessage::Severity::Error,
            "KISAK_VR_OUTPUT_SCALE",
            "Pimax Crystal Light packed mode requires an eye output scale of exactly 1.00.",
        });
    }
        ...
        scopeCaptureSize > 1024.0)
    {
        messages.push_back({
            ValidationMessage::Severity::Error,
            "KISAK_VR_SCOPE_CAPTURE_SIZE",
            "Pimax Crystal Light packed mode reserves a maximum 1024 px dedicated scope panel.",
        });
    }
    ...
    const std::string& revision)
{
    ...
    std::ostringstream output;
    output << "@echo off\r\n";
    output << "rem KisakCOD VR user settings - generated by beta.13 Configurator (Unified Setup/Compatibility)\r\n";
    ...
    SettingsMap* values)
{
    ...
    {
        Set(values, "VR_CUSTOM_MODE", "7684x3128");
        Set(values, "KISAK_VR_OUTPUT_SCALE", "1.00");
